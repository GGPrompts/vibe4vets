...
export function ExportDropdown({ resources }: ExportDropdownProps) {
  ...
  const exportToExcel = async () => {
    ...
    // Download
    XLSX.writeFile(wb, 'vibe4vets-saved-resources.xlsx');
  ...
  const exportToPDF = async () => {
    ...
    doc.text(`Exported from Vibe4Vets on ${new Date().toLocaleDateString()}`, margin, y);
    doc.setTextColor(0);
    y += 15;
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Page ${i} of ${pageCount} | Generated by Vibe4Vets`,
        pageWidth / 2,
    ...
    // Download
    doc.save('vibe4vets-saved-resources.pdf');
